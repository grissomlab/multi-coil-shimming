function Y = aftershim09(coefficients,slice,fieldmap,slice_ori)
% Y = AFTERSHIM09(COEFFICIENTS,SLICE)
% returns field strength ( in Hz ) of one slice after 9-coil-shim
% shim terms : Z0, Z, X, Y, Z2, ZX, ZY, X2-Y2, XY
% input:
...
for k = 1: size(index,1)
    ...
    X = onepoint09(shimR(index(k)),shimTheta(index(k))...
        ,shimPhi(index(k)));
    
    Y(index(k)) = shimField(index(k)) - X*coefficients; %%% X * coefficients are the corrections.
